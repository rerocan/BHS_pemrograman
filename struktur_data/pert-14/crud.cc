...
void update_user(const string& id, const string& name, const string& nim, const string& asal_kampus, const string& jurusan, const string& fakulltas){
    MYSQL* conn = connect_db();
    if (conn){
        stringstream query;
        query << "UPDATE mahasiswa SET name = '" << name << "', nim = '" << nim << "', asal_kampus = '" << asal_kampus << "', jurusan = '" << jurusan << "', fakultas = '" << fakulltas << "' WHERE id = " << id;
        if (mysql_query(conn, query.str().c_str())){
            cerr << "UPDATE failed." <<  mysql_error(conn) << endl;
        } else {
            cout << "User sucessfully updated " << endl;
        }
        mysql_close(conn);
    }
}
...
void delete_user(int user_id){
    ...
    if (conn){
        ...
        if (mysql_query(conn, query.str().c_str())){
            ...
            cout << "User sucessfully deleted " << endl;
        }
        mysql_close(conn);
    }
}
...
int main(){
    ...
    while (true){
        ...
        if (choice == 1){
            ...
            getline(cin, fakultas);
            update_user(to_string(user_id), name, nim, asal_kampus, jurusan, fakultas); 
        }else if (choice == 4){
            int user_id;
